...
% get variable names
hydro.headers = strsplit(fgets(fh),';');
...
assert(max(abs(hydro.data(:,1)-clim.data(:,1)))==0);
assert(max(abs(hydro.data(:,1)-geol.data(:,1)))==0);
assert(max(abs(hydro.data(:,1)-topo.data(:,1)))==0);
assert(max(abs(hydro.data(:,1)-vege.data(:,1)))==0);
assert(max(abs(hydro.data(:,1)-soil.data(:,1)))==0);

% pull only the data we want to regress against
geol.data  = geol.data( :,[2,3,6,7,8]);
...
hydro.data = hydro.data(:,2:end);

% pull corresponding headers
geol.headers  = geol.headers([2,3,6,7,8]);
...
hydro.headers = hydro.headers(2:end);
str = hydro.headers{end};
hydro.headers(end) = {str(1:end-1)};

% column labels
allHeaders = ['gauge_id',hydro.headers,geol.headers,clim.headers,topo.headers,soil.headers,vege.headers];

% aggregate inputs and targets
allData = [hydro.data,geol.data,clim.data,topo.data,soil.data,vege.data];
...
% file name
fname = strcat(extDir,'/camels_chars_v2.txt');
