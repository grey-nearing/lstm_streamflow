function [net,Yhat,stats,indexes] = lstm(X,Y,parms,lstmNodeSize,Xtest,Ytest,nBins,indexes)
...
Yhat = predict(model,Xtest{1});

% dimensions
assert(all(isequal(size(Yhat{1}),size(Ytest{1}))));

% stats
stats.NSE = nse(Yhat{1}',Ytest{1}');
stats.MI  = mutual_info(Yhat{1}',Ytest{1}',nBins);
