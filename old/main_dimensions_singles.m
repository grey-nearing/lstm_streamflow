...
%function main_dimensions_singles(hardware)
hardware = 'gpu';
...
% gpu groups
groupID = floor((1:nBasins)/(nProcs+1e-6))+1;
nGroups = length(unique(groupID));
...
%    end 
   ...
   % clear composites to reset spmd context
   clear tBasin compY compNSE compNMI r m k kTrain compModel b iGPU

   % save only trained models
   fname = strcat('./results/progress_',hardware,'.mat');
   save(fname,'-v7.3');

end % g-loop

% screen report
fprintf('Finished training all models; t = %f[s].\n',toc(tTrain));    
...
% save only trained models
fname = strcat('./results/dimensions_',hardware,'.mat');
save(fname,'-v7.3');
